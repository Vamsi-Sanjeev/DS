/*
  ...
  2. Security
    ...
    - Add policies for anonymous users to read/write public data
*/
...
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "Anyone can insert prediction data" ON prediction_data;
  DROP POLICY IF EXISTS "Anyone can read prediction data" ON prediction_data;
...
CREATE POLICY "Anyone can insert prediction data"
  ON prediction_data
  FOR INSERT
  TO anon
  WITH CHECK (true);

CREATE POLICY "Anyone can read prediction data"
  ON prediction_data
  FOR SELECT
  TO anon
  USING (true);
